...
Note: One thing you may notice is that all our class methods are now using the ruby keyword `self`. Like we mentioned yesterday you could use the class name instead, but we are going to keep to the ruby convention from now on.

In the start code, we have a `PizzaOrder` model and a `Customer` model and their corresponding tables. So at the moment, our models are completely unrelated. Pizza Orders know nothing about Customers and Customers know nothing about Pizza Orders. How can we relate a customer to a set of orders?

We want to create a one-to-many relationship between them, with one customer having many pizza orders, so we can use a foreign key in the pizza orders table referencing the customer id. We do this with the references keyword, and specify which column of another table it is linked to. Every time an order is created, it should be given a relevant customer's id that it is attached to.
...
CREATE TABLE pizza_orders (
  ...
  topping VARCHAR(255),
  quantity INT2,
  customer_id INT4 REFERENCES customers(id) // NEW
);
```
...
We have a customer model, let's check that we can use it to save some customers in the database.
...
Let's also call the `Customer.delete_all` method in `console.rb` before we create any new objects.
